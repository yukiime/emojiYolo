...
def user_config_dir(dir='Ultralytics', env_var='YOLOV5_CONFIG_DIR'):
    ...
    if env:
        ...
        cfg = {'Windows': 'F:/yolov5/roaming', 'Linux': '.config', 'Darwin': 'Library/Application Support'}  # 3 OS dirs
...
def check_imshow(warn=False):
    ...
    try:
        ...
        # OpenCV 的 imshow 方法显示一个测试图像/全黑图像 
...
def non_max_suppression(
    ...
    # xi:image index, x:image inference
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        # # 第一层过滤   根据conf_thres去除背景目标(obj_conf<conf_thres的目标)
